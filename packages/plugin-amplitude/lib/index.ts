...
export default class AmplitudeBrowserPlugin extends ItlyPluginBase {
  ...
  get amplitude(): any {
    return AmplitudeBrowserPlugin.getGlobalAmplitude();
  }
  ...
  constructor(apiKey: string, amplitudeOptions?: AmplitudeOptions) {
    super();
    if (!this.amplitude) {
      AmplitudeBrowserPlugin.initialize();
    }
    this.amplitude.getInstance().init(apiKey, undefined, amplitudeOptions);
  }
